...
const (
	Version                 = "1.0.0"
	DefaultConfigPath       = "/etc/qfw/qfw.conf"
	DefaultAPIPort          = ":8080"
...
func main() {
	...
	if flags.version {
		fmt.Printf("QFW - qFibre Firewall Manager v%s\n", Version)
		os.Exit(0)
	}
	...
	logger.Info("main", "QFW started successfully", "version", Version, "pid", os.Getpid())
	...
	// Graceful shutdown
	app.shutdown()
	logger.Info("main", "QFW shutdown completed")
}
...
func preFlightChecks() error {
	...
	if runtime.GOOS != "linux" {
		return fmt.Errorf("QFW requires Linux operating system")
	}
	...
	if os.Geteuid() != 0 {
		return fmt.Errorf("QFW requires root privileges")
	}
